...
def verif_cpf(cpf_bruto):
    cpf_ref = cpf_bruto.replace(".", "").replace("-", "")
    calculo = calculo_cpf(cpf_ref)
    if cpf_ref[9] == str(calculo):
        p1 = True
    
    calculo = calculo_cpf(cpf_ref, cont_regress=11)
    if cpf_ref[10] == str(calculo):
        p2 = True

    if p1 and p2 == True:
        return True

def calculo_cpf(cpf ,limite=1, cont_regress=10, calculo=0):
    for c in cpf:
        mult = int(c) * cont_regress
        calculo += mult
        cont_regress -= 1
        print(c, calculo)
        if cont_regress == limite:
            break
